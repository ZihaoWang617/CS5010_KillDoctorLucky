 ...
 * travel to any named room in the mansion, providing strategic positioning advantages.
 */
public class RoomCard extends Card {
     ...
     * 
     * @param name the display name of the card (e.g., "Kitchen Card", "Library Access")
     * @param targetRoom the name of the room this card teleports to
     * @throws IllegalArgumentException if name or targetRoom is null or empty
     */
    public RoomCard(String name, String targetRoom) {
        super(name, "A room card that allows instant teleportation to the " + targetRoom + ".");
        
        if (targetRoom == null || targetRoom.trim().isEmpty()) {
            throw new IllegalArgumentException("Target room cannot be null or empty");
        }
        
        this.targetRoom = targetRoom.trim();
    }
    ...
    /**
     * Executes this room card to teleport the player to the target room.
     * The player is instantly moved to the specified room regardless of distance
     * or normal movement restrictions. Playing this card removes it from the player's hand.
     * 
    ...
    public void execute(Game game, Player player) {
        ...
        // The Game class will get the target room via getTargetRoom() and move the player
    }
