 ...
 * have an attack value that contributes to the success of killing Doctor Lucky.
...
public class WeaponCard extends Card {
   ...
   * @param weaponAttackValue the strength of this weapon for murder attempts
   * @throws IllegalArgumentException if name is null/empty or attackValue is
   *                                  negative
   */
  public WeaponCard(String name, int weaponAttackValue) {
    super(name,
        "A weapon that can be used to attack Doctor Lucky with " 
        + weaponAttackValue + " attack power.");

    if (weaponAttackValue < 0) {
      throw new IllegalArgumentException("Attack value cannot be negative");
    }

    this.attackValue = weaponAttackValue;
  }

  /**
   * Gets the attack value of this weapon. This value is used to calculate the
  ...
  /**
   * Executes this weapon card during a murder attempt. The weapon's attack value
   * is added to the murder attempt's total damage. Playing this card removes it
